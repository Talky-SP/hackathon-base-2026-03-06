...
interface UseResizableOptions {
  ...
  side: 'left' | 'right';
  /** Width below which the panel snaps closed. Defaults to minWidth - 20. */
...
export function useResizable({
  ...
  const snap = snapThreshold ?? minWidth - 20;
  ...
  // Stable refs for min/max/snap/side so the listeners never need re-registration
  const configRef = useRef({ minWidth, maxWidth, side, snap });
  configRef.current = { minWidth, maxWidth, side, snap };

  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      if (!isDragging.current) return;
      const { minWidth: min, maxWidth: max, side: s, snap: sn } = configRef.current;
      const delta = s === 'left'
